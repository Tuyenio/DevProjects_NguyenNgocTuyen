...
class LibraryApp:
    ...
    def add_book(self):
        if not (self.entry_title.get() and self.entry_author.get() and self.validate_entry(self.entry_year.get()) and self.validate_entry(self.entry_isbn.get()) and self.validate_entry(self.entry_quantity.get())):
            messagebox.showinfo("Thông Báo", "Vui lòng nhập đầy đủ thông tin và Đảm bảo thông tin nhập vào phù hợp điều kiện nhập")
            return
        try:
    ...
    def update_borrow_tree(self):
        ...
        for idx, row in enumerate(cur.fetchall()):
            ...
            status = "Đã Cấp Sách Thành Công" if actual_return_date <= return_date else "Đã trả sách quá hạn"
    ...
    def show_statistics(self):
        ...
        stats_data = [
            ...
            ("Tổng số phiếu mượn", borrowed_count),
            ("Tổng số tài khoản admin trong hệ thống", len(borrow_records)),
    ...
    def print_statistics(self, stats_data):
        if messagebox.askyesno("Xác Nhận", "Bạn có muốn in ra thông tin thống kê không?"):
            # Mở hộp thoại chọn thư mục và tên file lưu
            ...
            if file_path:
                try:
                    ...
                    messagebox.showerror("Lỗi", "Không có quyền ghi file. Hãy chọn một thư mục khác.")
                except FileNotFoundError:
                    messagebox.showerror("Lỗi", "Đường dẫn không hợp lệ. Vui lòng thử lại.")
                except Exception as e:
                    messagebox.showerror("Lỗi", f"Không thể lưu file: {str(e)}")
            else:
                messagebox.showinfo("Thông Báo", "Bạn chưa chọn nơi lưu file")
